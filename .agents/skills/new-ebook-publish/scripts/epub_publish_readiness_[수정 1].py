...
#   1 content      — quality_gate v2 pass + fresh reviewer verdict
...
def check_content(book_dir: Path, fm: dict[str, str]) -> list[str]:
    ...
    if fm.get("content_quality_gate_version") != "2":
        failures.append("quality_gate v2 기준 검수 아님 — 구버전 pass는 무효, quality_gate.py 재실행 필요")
...
def check_artifact(book_dir: Path, fm: dict[str, str]) -> list[str]:
    ...
    colophon = next(
        (text for name, text in blobs.items() if "펴낸곳" in text or "colophon" in name.lower()),
        "",
    )
    if not colophon:

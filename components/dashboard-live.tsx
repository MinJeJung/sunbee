...
function ApprovalReviewCard({ operation }: { operation: DashboardSnapshotV2["operations"][number] }) {
  ...
  return <article className="review-card" id={`review-${operation.id}`}>
    ...
    <div className="review-copy"><p className="eyebrow">완성본 검수</p><h3>{artifact.title}</h3><p>{formatNumber(artifact.characterCount)}자 · 팩트체크 {artifact.factCheckScore}점 · EPUB {artifact.epubCheckScore}점{artifact.betaReadScore != null ? ` · 베타리더 ★${artifact.betaReadScore.toFixed(1)}` : ""}</p>{artifact.betaReadSummary ? <p className="review-beta-summary">{artifact.betaReadSummary}</p> : null}<div className="review-artifacts"><a className="artifact-link" href={`/api/artifacts/${operation.id}?kind=manuscript`} rel="noreferrer" target="_blank"><FileText size={17} /> 책 원고 보기</a><a className="artifact-link" href={`/api/artifacts/${operation.id}?kind=epub`}><DownloadSimple size={17} /> EPUB 다운로드</a></div><small className="review-guidance">표지 제목·목차·본문 흐름을 확인한 뒤 승인하거나 수정 요청을 보내세요.</small></div>

...
export default async function BookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><div className="book-detail"><Link className="back-link" href="/dashboard#catalog"><ArrowLeft size={16} /> 전체 도서 원장으로</Link><section className="book-hero">
    <div className="detail-cover">{book.cover && book.assetId ? <Image alt={`${book.title} 표지`} fill priority sizes="220px" src={catalogAssetUrl(book.id, "cover")} unoptimized /> : <span>표지 없음</span>}</div>
    ...
    <div className="detail-downloads">{book.manuscript && book.assetId ? <a className="button secondary" href={catalogAssetUrl(book.id, "manuscript")}><DownloadSimple size={17} /> 원고</a> : null}{book.fileFormat === "EPUB" && book.epub && book.assetId ? <a className="button primary" href={catalogAssetUrl(book.id, "epub")}><DownloadSimple size={17} /> EPUB 원본</a> : null}{book.fileFormat === "PDF" && book.pdf && book.assetId ? <a className="button primary" href={catalogAssetUrl(book.id, "pdf")}><DownloadSimple size={17} /> PDF 원본</a> : null}</div></div>
